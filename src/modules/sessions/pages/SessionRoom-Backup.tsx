...
import { getSubjectInfo, detectSubject } from '@/modules/exercises/utils/subject-detector';
...
const SessionRoom = () => {
  ...
  const [sessionData, setSessionData] = useState<SessionResponse | null>(
    (location.state as LocationState)?.sessionData || null
  );
  const [accessCode] = useState<string>(
    (location.state as LocationState)?.accessCode || ''
  );
  const [currentUser] = useState<any>(
    (location.state as LocationState)?.currentUser || user
  );
          ...
          <div className="flex items-center justify-center min-h-[400px]">
            <Card className="text-center p-8">
              <CardContent>
                ...
                <h3 className="text-xl font-semibold mb-2">Cargando sesión...</h3>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-muted">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-4">
          <Alert className={`${connectionStatus === 'connected' ? 'border-green-200 bg-green-50' : 
            ...
            <div className="flex items-center gap-2">
              ...
              <AlertDescription className={`${connectionStatus === 'connected' ? 'text-green-700' : 
                connectionStatus === 'connecting' ? 'text-yellow-700' : 
                'text-red-700'}`}>
                {connectionStatus === 'connected' ? 'Conectado a la sesión' :
        ...
        {/* Session Header */}
        <Card className="mb-6 bg-gradient-to-r from-primary/10 via-blue-50 to-purple-50 border-primary/20 shadow-lg">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    {sessionData.status === 'active' ? 'En Progreso' :
                     sessionData.status === 'waiting' ? 'Esperando' :
                     sessionData.status === 'finished' ? 'Finalizada' : 'Cancelada'}
                  </Badge>
                  <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    <Users className="h-4 w-4" />
                    {participantCount || sessionData.participants.length} participantes
                  </div>
                </div>
              </div>
